...
# Create mesh
channel = Rectangle(Point(0, 0), Point(10.0, 3.0))
circle = Circle(Point(5.0, 5.0), 4.0)
...
#Define boundaries
inflow   = 'near(x[0], 0)'
outflow  = 'near(x[0], 10.0)'
walls    = 'near(x[1], 0) || near(x[1], 3.0)'
circle   = 'on_boundary && x[0]>1.0 && x[0]<9.0 && x[1]>0.5 && x[1]<3.5'
#Define inflow profile
inflow_profile = ('0.1*(3.0-x[1])*x[1]', '0')
#Define boundary conditions
bcu_inflow = DirichletBC(V, Expression(inflow_profile, degree=2), inflow)
bcu_walls = DirichletBC(V, Constant((0, 0)), walls)
bcu_circle = DirichletBC(V, Constant((0, 0)), circle)
bcp_outflow = DirichletBC(Q, Constant(0), outflow)
bcu = [bcu_inflow, bcu_walls, bcu_circle]
bcp = [bcp_outflow]
...
ufile = File("n-s-1/u.pvd")
pfile = File("n-s-1/p.pvd")
#Time-stepping
t = 0
